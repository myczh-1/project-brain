...
Project Brain 会在读取时校验这些文件。如果助手写入了损坏的 JSON/NDJSON 或者字段结构不对，读取现在会直接报错，而不是静默把它当成“空数据”。
...
- **`.project-brain/` 文件损坏**：修复损坏的 JSON/NDJSON，不要直接删除文件；Project Brain 会把损坏数据视为错误。
