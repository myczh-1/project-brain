...
import { readChange } from '../storage/changes.js';
  ...
  it('normalizes string arrays and persists the change', async () => {
    ...
    const result = await createChange({
      ...
      change: {
        ...
        affected_areas: [' package.json ', ' packages/core-protocol '],
    ...
    expect(result.change.affected_areas).toEqual(['package.json', 'packages/core-protocol']);
