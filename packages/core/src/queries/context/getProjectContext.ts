...
export async function projectContext(input: ProjectContextInput, storage: StoragePort, git: GitPort): Promise<ProjectContextOutput> {
  ...
  const decisions = storage.readDecisions(cwd).slice(-5).reverse();
  const progress = storage.readProgress(cwd).slice(-5).reverse();
  const milestones = storage.readMilestones(cwd);
  const notes = storage.readNotes(cwd);
  const recentCommits = git.parseLog(5, cwd);
  const hotPaths = git.calculateHotPaths(recentCommits);
